...
DELETE FROM `{{ params.project_id }}.{{ params.dataset_id }}.clean`
WHERE DATE(start_time_dts) = DATE('{{ ds }}');

-- 2. Transformation & Insert
INSERT INTO `{{ params.project_id }}.{{ params.dataset_id }}.clean`
SELECT
  sd.trip_uid,
  ...
  END AS day_type
FROM
  `{{ params.project_id }}.{{ params.dataset_id }}.raw` AS sd
  JOIN
  `{{ params.project_id }}.{{ params.dataset_id }}.stops` AS s
