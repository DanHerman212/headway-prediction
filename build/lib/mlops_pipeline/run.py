...
Builds a source wheel of mlops_pipeline and uploads it to GCS so that each
pipeline component can install fresh source code at runtime — no Docker image
rebuild needed for code changes.

Optionally triggers a Cloud Build to rebuild the training container image
with Docker layer caching (--build flag) when *dependencies* change.
"""
...
WHEEL_BUCKET = "mlops-artifacts-realtime-headway-prediction"
...
def _build_training_image() -> None:
    ...
    Only needed when requirements.txt changes.  Source code is delivered
    via a GCS wheel, so code-only changes never require a rebuild.
    """
    print(f"\n{'='*60}")
...
def _build_and_upload_wheel(run_name: str) -> str:
    """Build a source wheel of the project and upload it to GCS.

    Returns the GCS URI of the uploaded wheel.  Each pipeline component
    downloads and installs this wheel at startup, ensuring they always
    run the latest source code without a Docker image rebuild.
    """
    import glob

    # Build the wheel from the project root
    project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    dist_dir = os.path.join(tempfile.gettempdir(), f"wheel_{run_name}")
    ...
    if not whl_files:
        print(f"No .whl file found in {dist_dir}")
        sys.exit(1)
    local_whl = whl_files[0]
    whl_name = os.path.basename(local_whl)

    # Upload to GCS under the run directory
    gcs_blob_path = f"pipeline_runs/{run_name}/{whl_name}"
    gcs_uri = f"gs://{WHEEL_BUCKET}/{gcs_blob_path}"

    client = gcs_storage.Client()
    bucket = client.bucket(WHEEL_BUCKET)
    blob = bucket.blob(gcs_blob_path)
    blob.upload_from_filename(local_whl)
    print(f"Wheel uploaded: {gcs_uri}")
...
def main():
    ...
    # ---- Build and upload source wheel to GCS ------------------------------
    source_whl_uri = _build_and_upload_wheel(run_name)
    ...
    print(f"Source wheel:    {source_whl_uri}")
